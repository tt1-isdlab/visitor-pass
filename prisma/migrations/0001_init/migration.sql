-- RoboFest 2.0 Visitor Pass Registration — baseline schema
-- NOTE: this exact SQL was already applied directly to the provisioned
-- Supabase project. If you are setting up a fresh database, run
-- `npx prisma migrate deploy` to apply it. If pointing at the existing
-- Supabase project, instead run:
